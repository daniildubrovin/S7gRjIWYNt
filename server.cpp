...
void Server::handlingHttp(Socket socket, const std::vector<HttpMapping> &httpMappings) {
    Request request;
    try {
        request = parseHttp(readDataFromSocket(socket));
        Response response;
        ...
        Log::i("end mapping: " + httpMapping.url);
        ...
        Log::e(e.what());
        Log::i("start print error to html");
...
void Server::parseMultiPartBody(std::istringstream &iss, Server::Request &request) {
    ...
    while(!iss.eof()){
        string contentDisposition, contentType, fileType, fileName, name ;
        ...
        if(contains(line, "Content-Disposition")) {
            ...
            if(contains(contentDisposition, "name")){
                name = contentDisposition.substr(contentDisposition.find("name=\"") + 6);
        ...
        if(fileName.empty() && name.empty()) throw std::runtime_error("must be filename or name in body");
        if(fileName.empty()) {
            string value;
            ...
            request.params.insert(std::make_pair(name, value));
        }
        else {
            ...
            File f {fileName + fileType, contentType, tmpName, (int64_t)std::filesystem::file_size("tmp/" + tmpName)};
            request.files.insert(std::make_pair(tmpName, f));
        }
    }
}
...
void Server::clearTmpDir(Request& request) {
    for (const auto & entry : std::filesystem::directory_iterator("tmp/")) {
        string file = entry.path().string();
        if(request.files.count(file.substr(4))) remove(file.c_str());
    }
}
...
string Server::readDataFromSocket(Socket socket) {
    ...
    while(true){
        ...
        if(count != 0 && count > content_len) throw std::runtime_error("number bytes of data is bigger Content-Length");
    }
